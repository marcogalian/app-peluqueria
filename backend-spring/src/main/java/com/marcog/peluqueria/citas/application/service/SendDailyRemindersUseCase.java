...
@Slf4j
public class SendDailyRemindersUseCase {
    ...
    public void sendReminders() {
        ...
        List<Cita> citasDeManana = citaRepositoryPort.findByCriteria(start, end, null);
        ...
        for (Cita cita : citasPendientes) {
            for (NotificationPort port : notificationPorts) {
                try {
                    port.enviarRecordatorio(cita);
                } catch (Exception e) {
                    log.error("Error al enviar notificación con el puerto {}: {}", port.getClass().getSimpleName(),
                            e.getMessage());
                }
            }
        }
